...
class Drop_buoy():
    def __init__(self) :
        ...
        self.order="%"
        self.position="%"
        self.drop_auto=True#autorization to finish the move
    def which_state(self) :
        if self.state=="un_active":
            self.un_active()
        elif self.state=="su" :
    ...
    def sh(self) :
        global pump,servo,slide
        if slide.done_order==True and self.drop_auto==True:
...
def which_state() :#put grab in last !!
    ...
    print("slide",slide.state,"servo",servo.state,"pump",pump.state,"drop_buoy",drop_buoy.state)

drop_buoy=Drop_buoy()
drop_buoy.order="drop"
drop_buoy.color="red"
drop_buoy.position=1
while 1:
    which_state()
